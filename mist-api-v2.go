...
// MistApiV2ListSubnets List subnets
func MistApiV2ListSubnets(paramNetwork string, params *viper.Viper) (*gentleman.Response, map[string]interface{}, cli.CLIOutputOptions, error) {
	handlerPath := "list-subnets"
	if mistApiV2Subcommand {
		handlerPath = "Mist CLI " + handlerPath
	}
...
func MistApiV2GetSubnet(paramNetwork string, paramSubnet string, params *viper.Viper) (*gentleman.Response, map[string]interface{}, cli.CLIOutputOptions, error) {
	...
	return resp, decoded, cli.CLIOutputOptions{[]string{"name", "cloud", "network", "machines", "tags"}, []string{"id", "name", "cloud", "location", "external_id", "subnets", "machines", "tags", "owned_by", "created_by"}, []string{}, []string{}}, nil
}
...
func mistApiV2Register(subcommand bool) {
	...
	func() {
		...
		cmd := &cobra.Command{
			Use:     "list-subnets NETWORK",
			Short:   "List subnets",
			Long:    cli.Markdown("List subnets owned by the active org. READ permission required on network & cloud."),
			Hidden:  true,
			Example: examples,
			Args:    cobra.MinimumNArgs(1),
			Run: func(cmd *cobra.Command, args []string) {

				_, decoded, outputOptions, err := MistApiV2ListSubnets(args[0], params)
				if err != nil {
					logger.Fatalf("Error calling operation: %s", err.Error())
				}
	...
	func() {
		...
		cmd := &cobra.Command{
			...
			Run: func(cmd *cobra.Command, args []string) {
				...
				_, decoded, outputOptions, err := MistApiV2GetSubnet(args[0], args[1], params)
				if err != nil {
					logger.Fatalf("Error calling operation: %s", err.Error())
				}
	...
	func() {
		...
		cmd.SetErr(os.Stderr)
		cmd.SetUsageTemplate("/api/v2#" + strings.ToLower("Get") + "-" + strings.ReplaceAll(strings.ReplaceAll("/api/v2/networks/{network}/subnets", "{", "-"), "}", "-"))
		cmd.SetUsageFunc(customUsageFunc)
		getCmd.AddCommand(cmd)
