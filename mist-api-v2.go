...
func mistApiV2Register(subcommand bool) {
	...
	func() {
		...
		cmd := &cobra.Command{
			Use:     "list-users",
			Short:   "List users",
			Long:    cli.Markdown("Return current user if all parameter is false. Return all users if all paramater is True and user is an admin."),
			Example: examples,
			Group:   "users",
			Args:    cobra.MinimumNArgs(0),
			Run: func(cmd *cobra.Command, args []string) {
