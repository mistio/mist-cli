...
func MistApiV2ListOrgs(params *viper.Viper) (*gentleman.Response, map[string]interface{}, cli.CLIOutputOptions, error) {
	...
	paramAll := params.GetString("all")
	if paramAll != "" {
		req = req.AddQuery("all", fmt.Sprintf("%v", paramAll))
	}
	paramSearch := params.GetString("search")
	if paramSearch != "" {
		req = req.AddQuery("search", fmt.Sprintf("%v", paramSearch))
...
func mistApiV2Register(subcommand bool) {
	...
	func() {
		...
		cmd := &cobra.Command{
			Use:     "list-orgs",
			Short:   "List orgs",
			Long:    cli.Markdown("List orgs owned by the requester. If parameter all is True and requester is an admin then all orgs will be listed."),
			Example: examples,
			Group:   "orgs",
			Args:    cobra.MinimumNArgs(0),
			Run: func(cmd *cobra.Command, args []string) {
		...
		cmd.Flags().String("all", "", "Return all existing organizations")
		cmd.Flags().String("search", "", "Only return results matching search filter")
		cmd.Flags().String("sort", "", "Order results by")
		cmd.Flags().String("start", "", "Start results from index or id")
