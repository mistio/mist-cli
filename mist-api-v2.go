	...
	func() {
		...
		cmd := &cobra.Command{
			...
			Run: func(cmd *cobra.Command, args []string) {
				...
				resp, decoded, outputOptions, err := MistApiV2RunScript(args[0], params, body)
				if err != nil {
					logger.Fatalf("Error calling operation: %s", err.Error())
				}
				...
				reqParams["script"] = args[0]

				var actual interface{}
				if params.GetBool("wait") {
					wparams := viper.New()
					actual, err = cli.GetMatchValue(resp.Context, "response.body#job_id", reqParams, decoded)
					if err != nil {
						logger.Fatalf("Could not get matcher value: %s", err.Error())
					}
